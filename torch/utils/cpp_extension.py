...
class BuildExtension(build_ext, object):
    ...
    compilation compared to the standard ``setuptools.build_ext``. If Ninja
    cannot be found or if we are on an unsupported system, then we raise a
    warning and fallback to ``use_ninja = False``.
    '''
    ...
    def __init__(self, *args, use_ninja=True, **kwargs):
        super(BuildExtension, self).__init__(*args, **kwargs)
        self.no_python_abi_suffix = kwargs.get("no_python_abi_suffix", False)

        # Check if we can actually use ninja and warn+fallback if we can't.
        self.use_ninja = use_ninja
        if use_ninja:
            msg = ('Attempted to use ninja as the BuildExtension backend '
                   'but {}. Falling back to using the slow distutils backend.')
            if IS_WINDOWS:
                warnings.warn(msg.format('we don\'t support this on windows yet'))
                self.use_ninja = False
            elif not _system_has_ninja():
