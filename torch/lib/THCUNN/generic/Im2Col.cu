...
void THNN_(Im2Col_updateGradInput)(
  ...
  int ndim = THCTensor_(nDimension)(state, input);
  THArgCheck(ndim == 3 or ndim == 4, 2,
             "input should be 3D or 4D but got: %dD", ndim);

  int dimH = (ndim == 3) ? 1 : 2;
  int inputHeight  = THCTensor_(size)(state, input, dimH);
  int inputWidth   = THCTensor_(size)(state, input, dimH + 1);
